    ...
    def decide(
        ...
        if len(sources) < min_sources:
            return ResearchDecision(
                "SEARCHING",
                f"insufficient sources retained ({len(sources)} < {min_sources})",
                False,
            )
        if not evidence:
        ...
        covered = {
            getattr(c, "plan_item_id", getattr(c, "id", ""))
            for c in claims
        ...
        if missing and len(sources) < 15:
            return ResearchDecision(
                "SEARCHING", f"missing plan coverage for {len(missing)} subquestion(s)", False
            )

        return ResearchDecision(
            "SYNTHESIZING", "plan coverage satisfied with verified evidence", True
        )
