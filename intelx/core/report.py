...
def render_report_markdown(
    ...
    """Render the official INTELX research intelligence report markdown."""
    ...
    if contradictions:
        ...
        for c in disputed_claims:
            ...
            contra_lines.append(f'- **Disputed Claim**: "{c_text}" [C:{c_id[:8]}] [S:{s_id[:8]}]')
    else:
    ...
    if sources:
        for s in sources:
            ...
            source_lines.append(
                f"- **[S:{s_id[:8]}]** {title} — `{domain}` (Tier: {tier}, "
