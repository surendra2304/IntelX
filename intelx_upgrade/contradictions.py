    ...
    def detect(self, claims) -> list[Conflict]:
        ...
        for i, a in enumerate(claims):
            for b in claims[i + 1 :]:
                ...
                if len(shared) < 1:
                    continue
                aa = self.normalized(a.statement)
                bb = self.normalized(b.statement)
                opposite = False
                reason = "opposing language"
                for positive, opposites in self.CONTRARY_WORDS.items():
                    if positive in aa and any(x in bb for x in opposites):
                        opposite = True
                        reason = f"{positive} vs contrary assertion"
                    if positive in bb and any(x in aa for x in opposites):
                        opposite = True
                        reason = f"{positive} vs contrary assertion"
