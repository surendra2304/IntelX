...
from .models import ResearchPlan, ResearchPlanItem, ResearchState, ResearchStatus, stable_id
...
class ResearchController:
    ...
    def transition(self, state: ResearchState, status: ResearchStatus) -> ResearchState:
        allowed = {
            ...
            ResearchStatus.COMPLETE: set(),
            ResearchStatus.FAILED: set(),
            ResearchStatus.CANCELLED: set(),
            ResearchStatus.INSUFFICIENT_EVIDENCE: set(),
        }
    ...
    def make_plan(self, state: ResearchState, questions: list[str]) -> ResearchState:
        items = tuple(
            ResearchPlanItem(stable_id(state.identity.research_id, q), q) for q in questions
        )
